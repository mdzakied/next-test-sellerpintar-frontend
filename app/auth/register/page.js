...
export default function RegisterPage() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (result.success) {
        toast.success(result.message, result.newUser);
        setTimeout(() => {
          redirect("/auth/login");
        }, 500);
      } else {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <select
                ...
                className={`mt-2 block w-full rounded-md border px-3 py-1.5 text-sm text-gray-900 shadow-sm outline-none focus:ring-2 focus:ring-blue-600 cursor-pointer ${
